...
class TestGraphClient(GraphTestCase):
    async def test1_execute_batch_get_requests(self):
    ...
    async def test2_build_resource_path(self):
        drive = await self.client.me.drive.root.get().execute_query()
        self.assertEqual("/me/drive/items/{0}".format(drive.id), str(drive.resource_path))

    async def test3_build_url_resource_path(self):
        path = UrlPath(
            "Sample.docx",
    ...
    async def test4_build_url_nested_resource_path(self):
    ...
    async def test5_resolve_drive_url_path(self):
    ...
    async def test6_resolve_drive_children_path(self):
        path = self.client.me.drive.root.children.resource_path
        item_id = uuid.uuid4().hex
        path.patch(item_id)
        self.assertEqual(f"/me/drive/items/{item_id}", str(path))

    async def test7_build_drive_children_path(self):
        item_id = uuid.uuid4().hex
    ...
    async def test9_resolve_site_url_path(self):
        site = await self.client.sites.get_by_url(test_team_site_url).execute_query()
        self.assertEqual(f"{str(self.client.sites.resource_path)}/{site.id}", str(site.resource_path))

    async def test_10_resolve_drive_root_path(self):
        path = self.client.me.drive.root.resource_path
        item_id = uuid.uuid4().hex
        path.patch(item_id)
        self.assertEqual(f"/me/drive/items/{item_id}", str(path))

    async def test_11_build_site_root_path(self):
        site = await self.client.sites.root.get().execute_query()
        self.assertEqual(f"/sites/{site.id}", str(site.resource_path))

    async def test_12_resolve_term_children_path(self):
    ...
    async def test_13_resolve_me_resource_path(self):
        current_user = await self.client.me.get().execute_query()
        self.assertEqual("/users/{0}".format(current_user.id), str(current_user.resource_path))

    async def test_15_resolve_my_drive_resource_path(self):
        my_drive = await self.client.me.drive.get().execute_query()
        self.assertEqual("/drives/{0}".format(my_drive.id), str(my_drive.resource_path))

    async def test_16_resolve_entity_type_name(self):
        name = self.client.me.joined_teams.entity_type_name
        self.assertEqual("Collection(microsoft.graph.team)", name)

    async def test_17_build_path_from_url(self):
