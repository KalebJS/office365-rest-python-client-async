...
class TestSharePointSharing(TestCase):
    ...
    def setUpClass(cls):
        ...
        async def _async_setup():
            await client.web.lists.ensure_site_pages_library().execute_query()
            current_user = await client.web.current_user.get().execute_query()
    ...
    async def test3_get_file_sharing_info(self):
        ...
        sharing_info = await list_item.get_sharing_information().execute_query()
    ...
    async def test6_unshare_file(self):
        target_file_item = self.client.web.get_list_item("/SitePages/Home.aspx")
        result = await target_file_item.unshare().execute_query()
    ...
    async def test8_unshare_web(self):
        result = await self.client.web.unshare().execute_query()
        self.assertIsInstance(result, SharingResult)
