...
class TestSharePointGroup(SPTestCase):
    ...
    def setUpClass(cls):
        ...
        async def _async_setup():
            cls.target_user = await cls.client.web.ensure_user(test_user_principal_name).execute_query()
    ...
    async def test2_add_user_to_group(self):
        target_user = await self.__class__.target_group.users.add_user(self.target_user.login_name).execute_query()
    ...
    async def test4_expand_to_principals(self):
        result = await self.__class__.target_group.expand_to_principals().execute_query()
        self.assertIsNotNone(result.value)

    async def test5_remove_user_from_group(self):
        result = await self.__class__.target_group.users.remove_by_id(self.target_user.id).execute_query()
