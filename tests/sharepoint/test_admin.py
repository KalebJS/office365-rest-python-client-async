...
class TestAdmin(TestCase):
    ...
    async def test1_get_analytics_usage(self):
        ...
        analytics = await SPAnalyticsUsageService(self.client).get().execute_query()
    ...
    async def test5_get_file_version_policy(self):
        result = await self.tenant.get_file_version_policy().execute_query()
        self.assertIsNotNone(result.value)

    async def test6_clear_file_version_policy(self):
        result = await self.tenant.clear_file_version_policy().execute_query()
        self.assertIsNotNone(result.resource_path)
    ...
    async def test8_get_root_site_url(self):
        result = await self.tenant.get_root_site_url().execute_query()
        self.assertIsNotNone(result.value)
