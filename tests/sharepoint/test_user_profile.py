...
class TestUserProfile(TestCase):
    ...
    async def test2_get_profile_loader(self):
        user_profile = await self.my_client.profile_loader.get_user_profile().execute_query()
    ...
    async def test4_get_user_props(self):
        target_user = await self.my_client.web.ensure_user(test_user_principal_name).execute_query()
        result = await self.my_client.people_manager.get_user_profile_properties(target_user.login_name).execute_query()
        self.assertIsNotNone(result.value)

    async def test5_get_properties_for(self):
        me = self.my_client.web.current_user
        properties = await self.my_client.people_manager.get_properties_for(me).execute_query()
        self.assertIsNotNone(properties)

    async def test6_get_default_document_library(self):
        me = self.my_client.web.current_user
        result = await self.my_client.people_manager.get_default_document_library(me).execute_query()
        self.assertIsNotNone(result.value)

    async def test7_get_people_followed_by(self):
        me = await self.my_client.web.current_user.get().execute_query()
        result = await self.my_client.people_manager.get_people_followed_by(me.login_name).execute_query()
        self.assertIsNotNone(result)

    async def test7_start_stop_following(self):
        people_manager = PeopleManager(self.my_client)
        target_user = await self.my_client.web.ensure_user(test_user_principal_name).execute_query()
        result = await people_manager.am_i_following(target_user.login_name).execute_query()
        if result.value:
    ...
    async def test8_get_followers_for(self):
        target_user = await self.my_client.web.ensure_user(test_user_principal_name).execute_query()
        col = await self.my_client.people_manager.get_followers_for(target_user.login_name).execute_query()
    ...
    async def test_11_get_user_profile_properties(self):
        user_props = await self.my_client.web.current_user.get_user_profile_properties().get().execute_query()
    ...
    async def test_14_add_site_link(self):
        ...
        await PromotedSites.add_site_link(self.my_client, "https://www.google.com", "Google").execute_query()

    async def test_15_get_promoted_links_as_tiles(self):
        from office365.sharepoint.userprofiles.promoted_sites import PromotedSites

        result = await PromotedSites.get_promoted_links_as_tiles(self.my_client).execute_query()
        self.assertIsNotNone(result.value)
    ...
    async def test_16_get_promoted_links_as_tiles(self):
        ...
        after_result = await PromotedSites.get_promoted_links_as_tiles(self.my_client).execute_query()
