...
class TestView(SPTestCase):
    ...
    def setUpClass(cls):
        ...
        async def _async_setup():
            ...
            cls.target_field = await cls.target_list.fields.add(field_info).execute_query()
    ...
    async def test5_get_default_view_items(self):
        view_items = await self.target_list.default_view.get_items().get().execute_query()
    ...
    async def test7_update_view(self):
        ...
        await view_to_update.set_property("Title", title_updated).update().execute_query()

        result = await self.target_list.views.filter("Title eq '{0}'".format(title_updated)).get().execute_query()
        self.assertEqual(len(result), 1)

    async def test8_get_view_fields(self):
        view = await self.__class__.target_view.expand(["ViewFields"]).get().execute_query()
    ...
    async def test9_add_view_field(self):
        ...
        await self.__class__.target_view.view_fields.add_view_field(field_name).execute_query()
        after_view_fields = await self.__class__.target_view.view_fields.get().execute_query()
        self.assertEqual(self.__class__.view_fields_count + 1, len(after_view_fields))

    async def test_10_move_view_field_to(self):
        field_name = self.__class__.target_field.internal_name
        await self.__class__.target_view.view_fields.move_view_field_to(field_name, 2).execute_query()
        after_view_fields = await self.__class__.target_view.view_fields.get().execute_query()
        self.assertEqual(after_view_fields[2], field_name)

    async def test_11_remove_view_field(self):
        field_name = self.__class__.target_field.internal_name
        await self.__class__.target_view.view_fields.remove_view_field(field_name).execute_query()
        after_view_fields = await self.__class__.target_view.view_fields.get().execute_query()
    ...
    async def test_13_get_view_changes(self):
        changes = await self.client.site.get_changes(ChangeQuery(view=True)).execute_query()
