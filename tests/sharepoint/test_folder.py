...
class TestSharePointFolder(SPTestCase):
    ...
    async def test2_enum_folders(self):
        folders = await self.parent_folder.folders.get().execute_query()
        self.assertGreater(len(folders), 1)
        for child_folder in folders:
            self.assertIsNotNone(child_folder.resource_path)

    async def test4_get_folder_by_id(self):
        folder_id = self.__class__.input_folder.unique_id
        folder = await self.client.web.get_folder_by_id(folder_id).get().execute_query()
    ...
    async def test9_list_files(self):
    ...
    async def test_11_rename_folder(self):
        folder = self.__class__.input_folder
        new_folder_name = create_unique_name("Renamed_")
        folder = await folder.rename(new_folder_name).get().execute_query()
        self.assertEqual(new_folder_name, folder.name)

    async def test_12_move_folder(self):
        folder = self.__class__.input_folder
        folder_to = await folder.move_to(self.__class__.output_folder).execute_query()
        self.assertIsNotNone(folder_to.serverRelativeUrl)

    async def test_13_recycle_folder(self):
        result = await self.__class__.input_folder.recycle().execute_query()
    ...
    async def test_15_get_folder_changes(self):
        changes = await self.__class__.input_folder.get_changes().execute_query()
        self.assertIsInstance(changes, ChangeCollection)
        self.assertGreaterEqual(len(changes), 0)

    async def test_16_delete_folders(self):
