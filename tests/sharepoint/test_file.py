...
class TestSharePointFile(SPTestCase):
    ...
    async def test4_get_file_from_absolute_url(self):
        ...
        file = await File.from_url(result.value).with_credentials(test_client_credentials).get().execute_query()
    ...
    async def test6_load_file_metadata(self):
        list_item = await self.__class__.file.listItemAllFields.expand(["File"]).get().execute_query()
    ...
    async def test8_update_file_content(self):
        file = await self.__class__.file.save_binary_stream(self.text_content).execute_query()
    ...
    async def test_15_copy_file(self):
        copied_file = await self.__class__.file.copyto(self.folder_to, True).execute_query()
    ...
    async def test_19_get_files_changes(self):
        changes = await self.__class__.file.listItemAllFields.get_changes(ChangeQuery(item=True)).execute_query()
    ...
    async def test_22_upload_large_file(self):
        ...
        size_1mb = 1000000
        file = await self.folder_from.files.create_upload_session(path, size_1mb).execute_query()
