...
class TestSharePointClient(TestCase):
    async def test1_connect_with_app_principal(self):
        ctx = ClientContext(test_site_url).with_credentials(test_client_credentials)
        result = await Web.get_context_web_information(ctx).execute_query()
        self.assertIsNotNone(result.value.WebFullUrl)

    async def test2_connect_with_app_principal_alt(self):
    ...
    async def test4_connect_with_user_credentials(self):
        ctx = ClientContext(test_site_url).with_credentials(test_user_credentials)
        result = await Web.get_context_web_information(ctx).execute_query()
        self.assertIsNotNone(result.value.WebFullUrl)

    async def test5_init_from_url(self):
    ...
    async def test6_connect_with_client_cert(self):
        pass

    async def test7_construct_get_request(self):
        pass

    async def test8_execute_multiple_queries_sequentially(self):
    ...
    async def test9_execute_get_batch_request(self):
        client = ClientContext(test_site_url).with_credentials(test_user_credentials)
        current_user = client.web.current_user
        client.load(current_user)
        current_web = client.web
    ...
    async def test_10_execute_update_batch_request(self):
        client = ClientContext(test_site_url).with_credentials(test_user_credentials)
        web = client.web
        ...
        updated_web = await client.web.get().execute_query()
        self.assertEqual(updated_web.properties["Title"], new_web_title)

    async def test_11_execute_get_and_update_batch_request(self):
        ...
        updated_list_item = await (
            client.web.get_file_by_server_relative_url(page_url).listItemAllFields.get().execute_query()
        )
        self.assertEqual(updated_list_item.properties["Title"], new_title)

    async def test_12_create_and_delete_batch_request(self):
        pass

    async def test_13_get_and_delete_batch_request(self):
        file_name = create_unique_file_name("TestFile", "txt")
        client = ClientContext(test_site_url).with_credentials(test_user_credentials)
        list_pages = client.web.lists.get_by_title("Documents")
        files = await list_pages.root_folder.files.get().execute_query()
        files_count_before = len(files)
        new_file = await list_pages.root_folder.upload_file(file_name, "-some content goes here-").execute_query()
    ...
    async def test_14_get_entity_type_name(self):
    ...
    async def test_15_build_query_options(self):
    ...
    async def test_16_ensure_property(self):
        client = ClientContext(test_site_url).with_credentials(test_user_credentials)
