...
class TestUtility(SPTestCase):
    ...
    async def test1_get_current_user_email_addresses(self):
        result = await Utility.get_current_user_email_addresses(self.client).execute_query()
    ...
    async def test4_expand_groups_to_principals(self):
        ...
        result = await Utility.expand_groups_to_principals(self.client, [owner_group.login_name], 10).execute_query()
        self.assertIsNotNone(result.value)

    async def test5_create_email_body_for_invitation(self):
        result = await Utility.create_email_body_for_invitation(self.client, "SitePages/Home.aspx").execute_query()
        self.assertIsNotNone(result.value)
