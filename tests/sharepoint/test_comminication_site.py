...
class TestCommunicationSite(TestCase):
    ...
    async def test2_is_comm_site(self):
        result = await self.__class__.target_site.is_comm_site().execute_query()
        self.assertIsNotNone(result.value)

    async def test3_set_as_home_site(self):
        result = await self.__class__.target_site.set_as_home_site().execute_query()
        self.assertIsNotNone(result.value)

    async def test4_is_valid_home_site(self):
        result = await self.__class__.target_site.is_valid_home_site().execute_query()
        self.assertIsNotNone(result.value)
    ...
    async def test9_delete_site(self):
