...
class TestSPList(SPTestCase):
    ...
    async def test3_has_library_unique_perms_chaining(self):
        default_lib = await (
            self.client.web.default_document_library().select(["HasUniqueRoleAssignments"]).get().execute_query()
        )
        self.assertFalse(default_lib.has_unique_role_assignments)
    ...
    async def test5_library_add_unique_perms(self):
        ...
        await target_lib.add_role_assignment(target_user, target_role_def).execute_query()

    async def test6_library_get_unique_perms(self):
        target_lib = self.client.web.default_document_library()
        target_user = self.client.web.current_user
        assignment = await target_lib.get_role_assignment(target_user).get().execute_query()
    ...
    async def test6_library_remove_unique_perms(self):
        ...
        target_user = self.client.web.current_user
        target_lib = self.client.web.default_document_library()
        await target_lib.remove_role_assignment(target_user, target_role_def).execute_query()
    ...
    async def test8_create_list(self):
        ...
        list_to_create = await self.client.web.lists.add(list_properties).execute_query()
    ...
    async def test9_read_list_by_title(self):
        list_to_read = await self.client.web.lists.get_by_title(self.target_list_title).get().execute_query()
        self.assertEqual(self.target_list_title, list_to_read.title)

    async def test_10_read_list_by_id(self):
        list_to_read = await self.client.web.lists.get_by_id(self.__class__.target_list.id).get().execute_query()
        self.assertEqual(self.target_list.id, list_to_read.id)

    async def test_11_read_list_fields(self):
        fields = await self.__class__.target_list.get_related_fields().get().execute_query()
        self.assertGreater(len(fields), 0)

    async def test_12_update_list(self):
        list_to_update = self.__class__.target_list
        self.target_list_title += "_updated"
        await list_to_update.set_property("Title", self.target_list_title).update().execute_query()

        result = await (
            self.client.web.lists.filter("Title eq '{0}'".format(self.target_list_title)).get().execute_query()
        )
        self.assertEqual(len(result), 1)

    async def test_13_get_list_permissions(self):
        current_user = self.client.web.current_user
        result = await self.__class__.target_list.get_user_effective_permissions(current_user).execute_query()
    ...
    async def test_15_delete_list(self):
        ...
        await self.client.web.lists.get_by_title(list_title).delete_object().execute_query()

        result = await self.client.web.lists.filter("Title eq '{0}'".format(list_title)).get().execute_query()
        self.assertEqual(len(result), 0)

    async def test_16_get_list_using_path(self):
        pages_list = await self.client.web.get_list_using_path("SitePages").execute_query()
    ...
    async def test_21_get_list_by_title(self):
        site_pages = await self.client.web.get_list_by_title("Site Pages").get().execute_query()
    ...
    async def test_23_render_list_data_as_stream(self):
        result = await self.client.web.get_list_by_title("Site Pages").render_list_data_as_stream().execute_query()
