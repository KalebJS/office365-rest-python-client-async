...
class TestRoles(SPTestCase):
    ...
    async def test1_create_role(self):
        ...
        result = await self.client.web.role_definitions.add(permissions, self.role_name).execute_query()
    ...
    async def test3_get_by_name(self):
        result = await self.client.web.role_definitions.get_by_name(self.role_name).get().execute_query()
        self.assertIsNotNone(result.resource_path)

    async def test4_add_role_assignment(self):
        target_user = self.client.web.current_user
        result = await self.client.web.add_role_assignment(target_user, self.__class__.target_object).execute_query()
        self.assertIsNotNone(result.resource_path)

    async def test5_remove_role_assignment(self):
        target_user = self.client.web.current_user
        result = await self.client.web.remove_role_assignment(target_user, self.__class__.target_object).execute_query()
        self.assertIsNotNone(result.resource_path)
