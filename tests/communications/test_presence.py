...
class TestPresence(GraphTestCase):
    ...
    async def test1_get_my_presence(self):
        my_presence = await self.client.me.presence.get().execute_query()
    ...
    async def test5_clear_my_presence(self):
        ...
        await my_presence.clear_user_preferred_presence().execute_query()
        self.assertIsNotNone(my_presence.resource_path)
