...
class TestOutlookUser(GraphTestCase):
    ...
    async def test5_enable_automatic_replies(self):
        ...
        await self.client.me.enable_automatic_replies_setting(
            status="Scheduled",
    ...
    async def test6_get_mailbox_settings(self):
        result = await self.client.me.select(["MailboxSettings"]).get().execute_query()
        self.assertIsNotNone(result.mailbox_settings)

    @requires_delegated_permission("MailboxSettings.ReadWrite")
    async def test7_disable_automatic_replies(self):
        await self.client.me.disable_automatic_replies_setting().execute_query()
