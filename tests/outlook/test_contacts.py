...
class TestOutlookContacts(GraphTestCase):
    ...
    async def test1_create_contact(self):
        result = await self.client.me.contacts.add(
            "Pavel",
            "Bansky",
    ...
    async def test2_list_contacts(self):
        result = await self.client.me.contacts.get().execute_query()
        self.assertGreaterEqual(len(result), 1)

    @requires_delegated_permission("Contacts.ReadWrite")
    async def test3_update_contact(self):
        contact = self.__class__.target_contact
        self.assertIsNotNone(contact.id)
        await contact.set_property("department", "Media").update().execute_query()

    @requires_delegated_permission("Contacts.ReadWrite")
    async def test4_delete_contact(self):
        contact_id = self.__class__.target_contact.id
        await self.__class__.target_contact.delete_object().execute_query()
        # verify
        contacts = await self.client.me.contacts.get().execute_query()
