...
class TestCalendar(GraphTestCase):
    ...
    async def test4_list_my_cal_permissions(self):
        result = await self.client.me.calendar.calendar_permissions.get().execute_query()
    ...
    async def test5_list_my_cal_view(self):
        ...
        result = await self.client.me.get_calendar_view(start_dt=start_time, end_dt=end_time).execute_query()
        self.assertIsNotNone(result.resource_path)

    async def test6_get_my_reminder_view(self):
        end_time = datetime.utcnow()
        start_time = end_time - timedelta(days=14)
        result = await self.client.me.get_reminder_view(start_dt=start_time, end_dt=end_time).execute_query()
        self.assertIsNotNone(result.value)
    ...
    async def test_10_get_cal(self):
        ...
        result = await self.client.me.calendars[cal_id].select(["name", "owner"]).get().execute_query()
    ...
    async def test_12_allowed_calendar_sharing_roles(self):
        result = await self.client.me.calendar.allowed_calendar_sharing_roles(test_user_principal_name).execute_query()
        self.assertIsNotNone(result.value)
