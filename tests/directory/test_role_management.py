...
class TestRoleManagement(GraphTestCase):
    ...
    async def test1_list_role_definitions(self):
        result = (
            await self.client.role_management.directory.role_definitions.get().execute_query()
        )
    ...
    async def test3_list_role_assignments(self):
        col = (
            await self.client.role_management.directory.role_assignments.get().execute_query()
        )
