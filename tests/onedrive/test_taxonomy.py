...
class TestTermStore(GraphTestCase):
    ...
    async def test1_list_groups(self):
        result = await self.target_store.groups.top(1).get().execute_query()
        self.assertLessEqual(len(result), 1)
        for group in result:
            self.assertIsNotNone(group.resource_path)

    async def test2_create_group(self):
        group_name = "Group_" + uuid.uuid4().hex
        new_group = await self.target_store.groups.add(group_name).execute_query()
        self.assertIsNotNone(new_group.resource_path)
        self.__class__.target_group = new_group

    async def test3_get_group_by_name(self):
        name = self.__class__.target_group.display_name
        group = await self.target_store.groups.get_by_name(name).get().execute_query()
        self.assertIsNotNone(group.resource_path)

    async def test4_create_set(self):
        set_name = "Set_" + uuid.uuid4().hex
        new_set = await self.target_group.sets.add(set_name).execute_query()
        self.assertIsNotNone(new_set.resource_path)
        self.__class__.target_set = new_set

    async def test5_list_sets(self):
        sets = await self.target_group.sets.get().execute_query()
        self.assertIsNotNone(sets.resource_path)
        self.assertGreaterEqual(1, len(sets))

    async def test6_create_term(self):
        label_name = "Term_" + uuid.uuid4().hex
        new_term = await self.target_set.children.add(label_name).execute_query()
        self.assertIsNotNone(new_term.resource_path)
        self.__class__.target_term = new_term

    async def test7_list_terms(self):
        terms = await self.target_set.terms.get().execute_query()
        self.assertIsNotNone(terms.resource_path)
        self.assertGreaterEqual(1, len(terms))

    async def test8_delete_term(self):
        await self.target_term.delete_object().execute_query()

    async def test9_delete_set(self):
        await self.target_set.delete_object().execute_query()

    async def test_10_delete_group(self):
        await self.target_group.delete_object().execute_query()
