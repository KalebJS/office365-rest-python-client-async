...
class TestExcelCharts(GraphTestCase):
    ...
    def setUpClass(cls):
        ...
        async def _async_setup():
            ...
            cls.worksheet = await cls.excel_file.workbook.worksheets["Weight loss tracker"].get().execute_query()
    ...
    async def test1_list_charts(self):
        result = await self.__class__.worksheet.charts.get().execute_query()
        self.assertIsNotNone(result.resource_path)

    async def test2_get_chart_by_name(self):
        result = await self.__class__.worksheet.charts["Weight Tracker"].get().execute_query()
        self.assertIsNotNone(result.resource_path)
        self.__class__.chart = result

    async def test3_get_image(self):
        result = await self.__class__.chart.image().execute_query()
