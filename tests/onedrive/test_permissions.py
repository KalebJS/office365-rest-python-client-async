...
class TestPermissions(TestCase):
    ...
    def setUpClass(cls):
        ...
        async def _async_setup():
            cls.target_drive_item = await client.sites.root.drive.root.create_folder(folder_name).execute_query()
            cls.client = client
    ...
    async def test1_create_anonymous_link(self):
        permission = await self.__class__.target_drive_item.create_link("view", "anonymous").execute_query()
        self.assertIsNotNone(permission.id)
        self.assertIsNotNone(permission.roles[0], "read")

    @requires_app_permission("Files.ReadWrite.All", "Sites.ReadWrite.All")
    async def test2_create_company_link(self):
        permission = await self.__class__.target_drive_item.create_link("edit", "organization").execute_query()
        self.assertIsNotNone(permission.id)
    ...
    async def test4_driveitem_list_permissions(self):
        permissions = await self.__class__.target_drive_item.permissions.get().execute_query()
        self.assertIsNotNone(permissions.resource_path)
        self.assertGreater(len(permissions), 0)

    @requires_app_permission("Files.Read.All", "Files.ReadWrite.All", "Sites.Read.All", "Sites.ReadWrite.All")
    async def test5_driveitem_get_permission(self):
        result = await self.__class__.target_drive_item.permissions.get().top(1).execute_query()
        self.assertEqual(len(result), 1)
        perm_id = result[0].id
        perm = await self.__class__.target_drive_item.permissions[perm_id].get().execute_query()
    ...
    async def test9_create_site_permission(self):
        ...
        new_site_permission = await self.client.sites.root.permissions.add(["write"], app).execute_query()
    ...
    async def test_10_list_site_permissions(self):
        site_permissions = await self.client.sites.root.permissions.get().execute_query()
